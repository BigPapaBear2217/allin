...
function openAll() {
    ...
    if (inHand) {
        dataHtmlAll += '<div class="buttonSkills" onmouseout="isOutNow()" onmouseover="isOverNow()" onclick="takeToHand()">'+language[37]+'</div>'
    }
    if (onShoulder) {
        dataHtmlAll += '<div class="buttonSkills" onmouseout="isOutNow()" onmouseover="isOverNow()" onclick="takeOnShoulder()">'+language[70]+'</div>'
    }
    for (var a in selectedSkills) {
        dataHtmlAll += '<div class="buttonSkills" onmouseout="isOutNow()" onmouseover="isOverNow()" onclick="makeSkill(\''+selectedSkills[a].skill+'\',\''+selectedSkills[a].name+'\')"><b>'+selectedSkills[a].rank+'</b> | '+selectedSkills[a].name+'</div>'
    }
    tableBodyAll.innerHTML = dataHtmlAll
...
function isOutNow() {isOver = false}
function isOverNow() {isOver = true}
function takeToHand(){
