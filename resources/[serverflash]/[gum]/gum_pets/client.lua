                            ...
                            if GetEntityHealth(targetEntity) ~= 0 then
                                ...
                                if canThis then
                                    ...
                                    if Citizen.InvokeNative(0x305C8DCD79DA8B0F, 0, `INPUT_CONTEXT_X`) then
                                        if not Citizen.InvokeNative(0x772A1969F649E902, GetEntityModel(targetEntity)) and GetEntityType(targetEntity) == 1 and not Citizen.InvokeNative(0x772A1969F649E902, GetEntityModel(targetEntity)) and Citizen.InvokeNative(0x97F696ACA466B4E0, targetEntity) == 1 then
...
RegisterNUICallback('makeFavorite', function(data,cb)
    TriggerEvent("guminputs:getInput", "Accept", "Write input", function(cb)
        local input = cb
        if input ~= "close" and tonumber(input) > 0 and tonumber(input) < 5 then
            for a,b in pairs(pet_list) do
                if b.selected == 1 then
                    TriggerServerEvent("gum_pets:makeFavorite", b.id, data.skill, input)
                end
            end
        end
    end)
...
RegisterNUICallback('makeInteraction', function(data, cb)
    local skillForMake = ""
    for a,b in pairs(pet_list) do
        if b.selected == 1 then
            for c,d in pairs(json.decode(b.favorite)) do
                if tonumber(c) == tonumber(data.interaction) then
                    skillForMake = d
                end
            end
        end
...
function TakeHand()
    if not inhand then
        ...
        if IsPedMale(PlayerPedId()) then
            ...
            AttachEntityToEntity(pet_model, PlayerPedId(), GetEntityBoneIndexByName(PlayerPedId(), "SH_BodyStrap_01"), 0.11, -0.11, -0.21,  0, 0, 0, false, false, true, false, 0, true, false, false)
        end
