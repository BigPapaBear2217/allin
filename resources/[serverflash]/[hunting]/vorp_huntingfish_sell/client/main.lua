...
function StartButchers() -- Loading Butchers Function
    for i, v in ipairs(Config.Butchers3) do
...
function awardQuality(quality, entity, horse, cb)
    ...
    for k, v in pairs(Config.Animals3) do
    ...
    if not skinFound then
        ...
        TriggerServerEvent("vorp_hunting:giveReward3", "pelt", {model=skinFound,quality=quality,entity=entity,horse=horse}, false)
		cb()
    end
end
...
function SellAnimal() -- Selling animal function
    ...
    if horse ~= nil and horse ~= false then
        ...
        if Citizen.InvokeNative(0xA911EE21EDF69DAF, horse) ~= false then
            ...
            if Config.Animals3[model2] ~= nil then -- Fallback for paying for non pelts
                alreadysoldanimal = true
                TriggerServerEvent("vorp_hunting:giveReward3", "carcass", {model=model2,entity=holding2}, false)
    ...
    if holding ~= false and alreadysoldanimal == false then -- Checking if you are holding an animal
        ...
        if Config.Animals3[model] ~= nil then -- Paying for animals
            alreadysoldanimal = true
			TriggerServerEvent("vorp_hunting:giveReward3", "carcass", {model=model,entity=holding}, false)
...
Citizen.CreateThread(function()
    while true do
        ...
        if horse ~= nil then
            ...
            if 2 > dist then
                local model = GetEntityModel(holding)
                if holding ~= false and Config.Animals3[model] == nil then
        ...
        if size > 0 then
            for index = 0, size - 1 do
                ...
                if event == 1376140891 then
                    ...
                    if model and Config.SkinnableAnimals3[model] ~= nil and playergate == true and bool_unk == 1 then
                        TriggerServerEvent("vorp_hunting:giveReward3", "skinned", {model=model}, true)
                    end
                end
            end
        end
