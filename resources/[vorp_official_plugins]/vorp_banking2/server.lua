...
AddEventHandler('vorp_bank:getinfo', function(name)
  ...
  exports["ghmattimysql"]:execute("SELECT invspace FROM bank_users WHERE charidentifier = @charidentifier AND name = @name"
    ...
    if result[1] then
      ...
      local Parameters = { ['name'] = name, ['identifier'] = identifier, ['charidentifier'] = charidentifier, ['invspace'] = invspace }
      exports.ghmattimysql:execute("INSERT INTO bank_users ( `name`,`identifier`,`charidentifier`,`invspace`) VALUES ( @name, @identifier, @charidentifier, @invspace)"
        , Parameters)
    end

    local bankinfo = { invspace = invspace, name = name }
  ...
  if money >= amountToPay then
    if nextslot <= maxslots then
      ...
      TriggerClientEvent("vorp:TipRight", _source,
        Config.language.success .. (costlot * amount) .. " | " .. nextslot .. " / " .. maxslots, 100000)
    else
      TriggerClientEvent("vorp:TipRight", _source,
        Config.language.maxslots .. " | " .. invspac .. " / " .. maxslots, 100000)
    end
  else
    TriggerClientEvent("vorp:TipRight", _source, Config.language.nomoney, 100000)
  end
end)
...
AddEventHandler('vorp_bank:depositcash', function(amount, name, bankinfo)
  ...
  if money >= amount then
    ...
    exports.ghmattimysql:execute("UPDATE bank_users Set money=money+@money WHERE charidentifier=@charidentifier AND name = @name"
      , Parameters)
    TriggerClientEvent("vorp:TipRight", _source, Config.language.youdepo .. amount, 100000)
    ...
    TriggerClientEvent("vorp:TipRight", _source, Config.language.invalid, 100000)
  end
...
AddEventHandler('vorp_bank:depositgold', function(amount, name, bankinfo)
  ...
  if money >= amount then
    ...
    exports.ghmattimysql:execute("UPDATE bank_users Set gold=gold+@gold WHERE charidentifier=@charidentifier AND name = @name"
      , Parameters)
    TriggerClientEvent("vorp:TipRight", _source, Config.language.youdepog .. amount, 100000)
...
AddEventHandler('vorp_bank:withcash', function(amount, name, bankinfo)
  ...
  exports["ghmattimysql"]:execute("SELECT money FROM bank_users WHERE charidentifier = @charidentifier AND name = @name"
    ...
    if money >= amount then
      ...
      TriggerClientEvent("vorp:TipRight", _source, Config.language.withdrew .. amount, 100000)
...
AddEventHandler('vorp_bank:withgold', function(amount, name, bankinfo)
  ...
  exports["ghmattimysql"]:execute("SELECT gold FROM bank_users WHERE charidentifier = @charidentifier AND name = @name",
    ...
    if gold >= amount then
      ...
      TriggerClientEvent("vorp:TipRight", _source, Config.language.withdrewg .. amount, 100000)
...
function Discord(title, name, description, location)
  local logs = "https://discord.com/api/webhooks/1032736688704864357/QLmOVZZxmhFXWNALd_FoMqHBIr7JnZtot0Z3jE8LfRsxUyi50-oVk9gPcpBwlMNppa17"
